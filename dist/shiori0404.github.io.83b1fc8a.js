(function(global, factory) {
    typeof exports === 'object' && typeof module !== 'undefined' ? module.exports = factory() : typeof define === 'function' && define.amd ? define(factory) : (global = global || self, global.Decimal = factory());
})(this, function() {
    'use strict';
    ...
    var powerOf10 = function() {
        ...
        // Faster AND simpler
        var powersOf10 = [];
        for(var i = NUMBER_EXP_MIN + 1; i <= NUMBER_EXP_MAX; i++)powersOf10.push(Number("1e" + i));
   ...
   */ var Decimal = /** @class */ function() {
       ...
       */ this.exponent = NaN;
            if (value === undefined) {
                this.m = 0;
                this.e = 0;
            } else if (value instanceof Decimal) this.fromDecimal(value);
        ...
        Decimal.pow10 = function(value) {
            if (Number.isInteger(value)) return ME_NN(1, value);
            return ME(Math.pow(10, value % 1), Math.trunc(value));
        };
        Decimal.pow = function(value, other) {
            // Fast track: 10^integer
            if (typeof value === "number" && value === 10 && typeof other === "number" && Number.isInteger(other)) return ME_NN(1, other);
     ...
     */ Decimal.efficiencyOfPurchase = function(cost, currentRpS, deltaRpS) {
        ...
        Decimal.randomDecimalForTesting = function(absMaxExponent) {
            ...
            if (Math.random() * 20 < 1) return ME_NN(0, 0);
     ...
     */ Decimal.prototype.fromMantissaExponent_noNormalize = function(mantissa, exponent) {
        ...
        Decimal.prototype.fromString = function(value) {
            if (value.indexOf("e") !== -1) {
                ...
                if (isNaN(this.m)) throw Error("[DecimalError] Invalid argument: " + value);
            }
            return this;
        };
        ...
        Decimal.prototype.toNumber = function() {
            ...
            if (this.e > NUMBER_EXP_MAX) return this.m > 0 ? Number.POSITIVE_INFINITY : Number.NEGATIVE_INFINITY;
            if (this.e < NUMBER_EXP_MIN) return 0;
             // SAFETY: again, handle 5e-324, -5e-324 separately
            if (this.e === NUMBER_EXP_MIN) return this.m > 0 ? 5e-324 : -0.;
            var result = this.m * powerOf10(this.e);
            if (!isFinite(result) || this.e < 0) return result;
            var resultRounded = Math.round(result);
            if (Math.abs(resultRounded - result) < ROUND_TOLERANCE) return resultRounded;
            return result;
        };
        ...
        Decimal.prototype.toString = function() {
            ...
            if (this.e >= EXP_LIMIT) return this.m > 0 ? "Infinity" : "-Infinity";
            if (this.e <= -EXP_LIMIT || this.m === 0) return "0";
            if (this.e < 21 && this.e > -7) return this.toNumber().toString();
        ...
        Decimal.prototype.toExponential = function(places) {
            ...
            if (isNaN(this.m) || isNaN(this.e)) return "NaN";
            if (this.e >= EXP_LIMIT) return this.m > 0 ? "Infinity" : "-Infinity";
            if (this.e <= -EXP_LIMIT || this.m === 0) return "0" + (places > 0 ? padEnd(".", places + 1, "0") : "") + "e+0";
             // two cases:
            // 1) exponent is < 308 and > -324: use basic toFixed
            // 2) everything else: we have to do it ourselves!
            if (this.e > NUMBER_EXP_MIN && this.e < NUMBER_EXP_MAX) return this.toNumber().toExponential(places);
        ...
        Decimal.prototype.toFixed = function(places) {
            if (isNaN(this.m) || isNaN(this.e)) return "NaN";
            if (this.e >= EXP_LIMIT) return this.m > 0 ? "Infinity" : "-Infinity";
            if (this.e <= -EXP_LIMIT || this.m === 0) return "0" + (places > 0 ? padEnd(".", places + 1, "0") : "");
             // two cases:
            // 1) exponent is 17 or greater: just print out mantissa with the appropriate number of zeroes after it
            // 2) exponent is 16 or less: use basic toFixed
            if (this.e >= MAX_SIGNIFICANT_DIGITS) return this.m.toString().replace(".", "").padEnd(this.e + 1, "0") + (places > 0 ? padEnd(".", places + 1, "0") : "");
        ...
        Decimal.prototype.toPrecision = function(places) {
            ...
            if (places > this.e) return this.toFixed(places - this.e - 1);
        ...
        Decimal.prototype.round = function() {
            if (this.e < -1) return new Decimal(0);
            if (this.e < MAX_SIGNIFICANT_DIGITS) return new Decimal(Math.round(this.toNumber()));
            return this;
        };
        Decimal.prototype.floor = function() {
            if (this.e < -1) return Math.sign(this.m) >= 0 ? new Decimal(0) : new Decimal(-1);
            if (this.e < MAX_SIGNIFICANT_DIGITS) return new Decimal(Math.floor(this.toNumber()));
            return this;
        };
        Decimal.prototype.ceil = function() {
            if (this.e < -1) return Math.sign(this.m) > 0 ? new Decimal(1) : new Decimal(0);
            if (this.e < MAX_SIGNIFICANT_DIGITS) return new Decimal(Math.ceil(this.toNumber()));
            return this;
        };
        Decimal.prototype.trunc = function() {
            if (this.e < 0) return new Decimal(0);
            if (this.e < MAX_SIGNIFICANT_DIGITS) return new Decimal(Math.trunc(this.toNumber()));
            return this;
        };
        Decimal.prototype.add = function(value) {
            ...
            var decimal = D(value);
            if (this.m === 0) return decimal;
            if (decimal.m === 0) return this;
            var biggerDecimal;
            var smallerDecimal;
            if (this.e >= decimal.e) {
                biggerDecimal = this;
            ...
            if (biggerDecimal.e - smallerDecimal.e > MAX_SIGNIFICANT_DIGITS) return biggerDecimal;
        ...
        Decimal.prototype.mul = function(value) {
            ...
            if (typeof value === "number") {
                if (value < 1e307 && value > -) return ME(this.m * value, this.e);
     ...
     */ Decimal.prototype.cmp = function(value) {
      ...
      3e100
           */ if (this.m === 0) {
                if (decimal.m === 0) return 0;
                if (decimal.m < 0) return 1;
                if (decimal.m > 0) return -1;
            }
            if (decimal.m === 0) {
            ...
            if (this.m > 0) {
                if (decimal.m < 0) return 1;
                if (this.e > decimal.e) return 1;
                if (this.e < decimal.e) return -1;
                if (this.m > decimal.m) return 1;
                if (this.m < decimal.m) return -1;
                return 0;
            }
            if (this.m < 0) {
        ...
        Decimal.prototype.lt = function(value) {
            var decimal = D(value);
            if (this.m === 0) return decimal.m > 0;
            if (decimal.m === 0) return this.m <= 0;
            if (this.e === decimal.e) return this.m < decimal.m;
            if (this.m > 0) return decimal.m > 0 && this.e < decimal.e;
     ...
     */ Decimal.prototype.eq_tolerance = function(value, tolerance) {
        ...
        Decimal.prototype.pow = function(value) {
            ...
            if (Number.isSafeInteger(temp)) {
                ...
                if (isFinite(newMantissa) && newMantissa !== 0) return ME(newMantissa, temp);
            ...
            if (isFinite(newMantissa) && newMantissa !== 0) return ME(newMantissa, newExponent);
             // return Decimal.exp(value*this.ln());
            var result = Decimal.pow10(numberValue * this.absLog10()); // this is 2x faster and gives same values AFAIK
            if (this.sign() === -1) {
                if (Math.abs(numberValue % 2) === 1) return result.neg();
                else if (Math.abs(numberValue % 2) === 0) return result;
        ...
        Decimal.prototype.exp = function() {
            ...
            if (-706 < x && x < 709) return Decimal.fromNumber(Math.exp(x));
        ...
        Decimal.prototype.sqrt = function() {
            if (this.m < 0) return new Decimal(Number.NaN);
            if (this.e % 2 !== 0) return ME(Math.sqrt(this.m) * 3.16227766016838, Math.floor(this.e / 2));
        ...
        Decimal.prototype.cbrt = function() {
            ...
            var mod = this.e % 3;
            if (mod === 1 || mod === -1) return ME(newMantissa * 2.154434690031883, Math.floor(this.e / 3));
            if (mod !== 0) return ME(newMantissa * 4.641588833612778, Math.floor(this.e / 3));
     ...
     */ Decimal.prototype.egg = function() {
        ...
        Decimal.prototype.dp = function() {
            ...
            if (this.exponent >= MAX_SIGNIFICANT_DIGITS) return 0;
...
//# sourceMappingURL=shiori0404.github.io.83b1fc8a.js.map
